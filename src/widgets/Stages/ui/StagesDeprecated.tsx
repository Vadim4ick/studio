...
import { useScroll, useTransform, motion, inView } from "framer-motion"
...
const Stages = () => {
  ...
  const refs = stages.map(() => useRef<HTMLDivElement | []>([]))

  // const inView2 = stages.map((_, idx) => {
  //   console.log(refs[0])

  //   // return useInView({ current: refs[idx].current }, {})
        ...
        <div className="sticky top-0 flex h-full w-[40%] pt-6">
          ...
          <div className="flex flex-col gap-[50px] pl-10">
            {stages.map((item) => (
              <div key={item.id}>
                {/* CIRCLE */}
                {/* <div className="absolute left-5 size-[23px] rounded-full bg-[#BDBDBD]" /> */}
                <motion.div
                  // style={inView[idx] === true ? { background: "#9E77ED" } : {}}
...
//   //   {
//   //     current: reftest.current,
//   //   },
//   //   {},
//   // )

//   const inView = stages.map((_, idx) =>
//     useInView(
//       {
//         current: refs[idx].current,
//       },
//       {},
//     ),
//   )
